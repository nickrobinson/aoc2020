...
func TestFindPossibleAdapterSets(t *testing.T) {
	...
	sets := FindPossibleAdapterSets(joltages, []int{22})
	if sets != 8 {
...
func TestFindPossibleAdapterSetsAdvanced(t *testing.T) {
	joltages := []int{0, 28, 33, 18, 42, 31, 14, 46, 20, 48, 47, 24, 23, 49, 45, 19, 38, 39, 11, 1, 32, 25, 35, 8, 17, 7, 9, 4, 2, 34, 10, 3}
	sets := FindPossibleAdapterSets(joltages, []int{52})
	if sets != 19208 {
